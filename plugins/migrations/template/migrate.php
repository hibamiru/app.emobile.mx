...
if( isset( $_GET['mdb-profile'] ) ){
	$loaded_profile = $this->get_profile( $_GET['mdb-profile'] );
}
else{
...
if( ! $is_default_profile ) {
	...
	*/
	...
	$loaded_profile = $this->maybe_update_profile( $loaded_profile, $_GET['mdb-profile'] );
}
...
<script type='text/javascript'>
	var mdb_default_profile = <?php echo ( $is_default_profile ? 'true' : 'false' ); ?>;
	<?php if( isset( $loaded_profile['select_tables'] ) && ! empty( $loaded_profile['select_tables'] ) ) : ?>
		var mdb_loaded_tables = <?php echo json_encode( $loaded_profile['select_tables'] ); ?>;
	<?php endif; ?>
	<?php if( isset( $loaded_profile['select_post_types'] ) ) : ?>
		var mdb_loaded_post_types = <?php echo json_encode( $loaded_profile['select_post_types'] ); ?>;
	<?php endif; ?>
	<?php if( isset( $loaded_profile['select_backup'] ) && ! empty( $loaded_profile['select_backup'] ) ) : ?>
		var mdb_loaded_tables_backup = <?php echo json_encode( $loaded_profile['select_backup'] ); ?>;
	<?php endif; ?>
	var mdb_convert_exclude_revisions = <?php echo ( $convert_exclude_revisions ? 'true' : 'false' ); ?>;
	var mdb_convert_post_type_selection = <?php echo ( $convert_post_type_selection ? '1' : '0' ); ?>;
</script>
...
<div class="migrate-tab content-tab">
	...
	<form method="post" id="migrate-form" action="#migrate" enctype="multipart/form-data">
		...
		<?php if( count( $this->settings['profiles'] ) > 0 ){ ?>
			<a href="<?php echo $this->plugin_base; ?>" class="return-to-profile-selection clearfix">&larr; <?php _e( 'Back to select a saved profile', 'migrations' ); ?></a>
		<?php } ?>
		...
		<div class="option-section">
			...
			<ul class="option-group migrate-selection">
				<li>
					...
					<ul>
						<li>
							...
							<?php _e( 'Save as file to your computer', 'migrations' ); ?>
							</label>
						</li>
						<?php if ( $this->gzip() ) : ?>
							<li>
								<label for="gzip_file">
									...
									<?php _e( 'Compress file with gzip', 'migrations' ); ?>
								</label>
							</li>
						<?php endif; ?>
					</ul>
				...
				<li class="pull-list">
					...
					<?php _e( 'Pull', 'migrations' ); ?><span class="option-description"><?php _e( 'Replace this site\'s db with remote db', 'migrations' ); ?></span>
					</label>
					<ul>
						<li></li>
					</ul>
				...
				<li class="push-list">
					...
					<?php _e( 'Push', 'migrations' ); ?><span class="option-description"><?php _e( 'Replace remote db with this site\'s db', 'migrations' ); ?></span>
					</label>
					<ul>
						<li></li>
					</ul>
			...
			<div class="connection-info-wrapper clearfix">
				<textarea class="pull-push-connection-info" name="connection_info" placeholder="<?php _e( 'Connection Info - Site URL &amp; Secret Key', 'migrations' ); ?>"><?php echo ( isset( $loaded_profile['connection_info'] ) ? $loaded_profile['connection_info'] : '' ); ?></textarea>
				<br />
			...
			<div class="notification-message warning-notice ssl-notice inline-message">
				<strong><?php _e( 'SSL Disabled', 'migrations' ); ?></strong> &mdash; <?php _e( 'We couldn\'t connect over SSL but regular http (no SSL) appears to be working so we\'ve switched to that. If you run a push or pull, your data will be transmitted unencrypted. Most people are fine with this, but just a heads up.', 'migrations' ); ?>
			</div>

		</div>

		<p class="connection-status"><?php _e( 'Please enter the connection information above to continue.', 'migrations' ); ?></p>

		<div class="notification-message error-notice different-plugin-version-notice inline-message" style="display: none;">
			<b><?php _e( 'Version Mismatch', 'migrations' ); ?></b> &mdash; <?php printf( __( 'We\'ve detected you have version <span class="remote-version"></span> of WP Sync DB at <span class="remote-location"></span> but are using %1$s here. Please go to the <a href="%2$s">Plugins page</a> on both installs and check for updates.', 'migrations' ), $GLOBALS['mdb_meta'][$this->plugin_slug]['version'], network_admin_url( 'plugins.php' ) ); ?>
		</div>

		<div class="notification-message error-notice directory-permission-notice inline-message" style="display: none;">
			<strong><?php _e( 'Cannot Access Uploads Directory', 'migrations' ); ?></strong> &mdash;
			<?php
				_e( 'We require write permissions to the standard WordPress uploads directory. Without this permission exports are unavailable. Please grant 755 permissions on the following directory:', 'migrations' );
		...
		<div class="step-two">
			...
			<div class="option-section">
				<div class="header-wrapper clearfix">
					<div class="option-heading find-heading"><?php _e( 'Find', 'migrations' ); ?></div>
					<div class="option-heading replace-heading"><?php _e( 'Replace', 'migrations' ); ?></div>
				</div>

				<p class="no-replaces-message"><?php _e( 'Doesn\'t look we have any replaces yet, <a href="#" class="js-action-link add-replace">add one?</a>', 'migrations' ); ?></p>
				...
				<div id="new-url-missing-warning" class="warning inline-message missing-replace"><?php printf( __( '<strong>New URL Missing</strong> &mdash; Please enter the protocol-relative URL of the remote website in the "New URL" field. If you are unsure of what this URL should be, please consult <a href="%s" target="_blank">our documentation</a> on find and replace fields.', 'migrations' ), 'https://deliciousbrains.com/migrations/documentation/#find-and-replace' ); ?></div>
				<div id="new-path-missing-warning" class="warning inline-message missing-replace"><?php printf( __( '<strong>New File Path Missing</strong> &mdash; Please enter the root file path of the remote website in the "New file path" field. If you are unsure of what the file path should be, please consult <a href="%s" target="_blank">our documentation</a> on find and replace fields.', 'migrations' ), 'https://deliciousbrains.com/migrations/documentation/#find-and-replace' ); ?></div>
				...
				<div class="indent-wrap expandable-content table-select-wrap" style="display: none;">
					...
					<ul class="option-group table-migrate-options">
						<li>
							...
							<?php _e( 'Migrate all tables with prefix', 'migrations' ); ?> "<span class="table-prefix"><?php echo $wpdb->prefix; ?></span>"
							</label>
						</li>
						<li>
							...
							<?php _e( 'Migrate only selected tables below', 'migrations' ); ?>
							</label>
						</li>
					</ul>
					...
					<div class="select-tables-wrap select-wrap">
						...
						<br />
						<a href="#" class="multiselect-select-all js-action-link"><?php _e( 'Select All', 'migrations' ); ?></a>
						<span class="select-deselect-divider">/</span>
						<a href="#" class="multiselect-deselect-all js-action-link"><?php _e( 'Deselect All', 'migrations' ); ?></a>
						<span class="select-deselect-divider">/</span>
						<a href="#" class="multiselect-invert-selection js-action-link"><?php _e( 'Invert Selection', 'migrations' ); ?></a>
					</div>
				</div>
			</div>
			...
			<div class="option-section" style="display: block;">
				...
				<?php _e( 'Exclude Post Types', 'migrations' ); ?>
				</label>
			...
			<div class="option-section">
				<div class="header-expand-collapse clearfix">
					<div class="expand-collapse-arrow collapsed">&#x25BC;</div>
					<div class="option-heading tables-header"><?php _e( 'Advanced Options', 'migrations' ); ?></div>
				</div>
				...
				<div class="indent-wrap expandable-content">
					...
					<ul>
						<li>
							...
							<?php _e( 'Replace GUIDs', 'migrations' ); ?>
							</label>

							<a href="#" class="general-helper replace-guid-helper js-action-link"></a>

							<div class="replace-guids-info helper-message">
								<?php printf( __( 'Although the <a href="%s" target="_blank">WordPress Codex emphasizes</a> that GUIDs should not be changed, this is limited to sites that are already live. If the site has never been live, I recommend replacing the GUIDs. For example, you may be developing a new site locally at dev.somedomain.com and want to migrate the site live to somedomain.com.', 'migrations' ), 'http://codex.wordpress.org/Changing_The_Site_URL#Important_GUID_Note' ); ?>
							</div>
						</li>
						<li>
							...
							<?php _e( 'Exclude spam comments', 'migrations' ); ?>
							</label>
						</li>
						<li class="keep-active-plugins">
							...
							<?php _e( 'Do not migrate the \'active_plugins\' setting (i.e. which plugins are activated/deactivated)', 'migrations' ); ?>
							</label>
						</li>
						<li>
			...
			<div class="option-section backup-options" style="display: block;">
				<label for="create-backup" class="backup-checkbox checkbox-label">
					...
					<?php _e( 'Backup the <span class="directory-scope">local</span> database before replacing it', 'migrations' ); ?><br />
					<span class="option-description backup-description"><?php _e( 'An SQL file will be saved to', 'migrations' ); ?> <span class="uploads-dir"><?php echo $this->get_short_uploads_dir(); ?></span></span>
				</label>
				...
				<div class="indent-wrap expandable-content">
					<ul>
						<li>
							...
							<?php _e( 'Backup all tables with prefix', 'migrations' ); ?> "<span class="table-prefix"><?php echo $wpdb->prefix; ?></span>"
							</label>
						</li>
						<li>
							...
							<?php _e( 'Backup only tables selected for migration', 'migrations' ); ?>
							</label>
						</li>
						<li>
							...
							<?php _e( 'Backup only selected tables below', 'migrations' ); ?>
							</label>
						</li>
					</ul>
			...
			<div class="option-section save-migration-profile-wrap">
				...
				<?php _e( 'Save Migration Profile', 'migrations' ); ?><span class="option-description"><?php _e( 'Save the above settings for the next time you do a similiar migration', 'migrations' ); ?></span>
				</label>
				...
				<div class="indent-wrap expandable-content">
					<ul class="option-group">
						...
						<li>
							...
							<?php _e( 'Create new profile', 'migrations' ); ?>
							</label>
			...
			<div class="notification-message warning-notice prefix-notice pull">
				<h4><?php _e( 'Warning: Different Table Prefixes', 'migrations' ); ?></h4>

				<p><?php _e( 'Whoa! We\'ve detected that the database table prefix differs between installations. Clicking the Migrate DB button below will create new database tables in your local database with prefix "<span class="remote-prefix"></span>".', 'migrations' ); ?></p>

				<p><?php printf( __( 'However, your local install is configured to use table prefix "%1$s" and will ignore the migrated tables. So, <b>AFTER</b> migration is complete, you will need to edit your local install\'s wp-config.php and change the "%1$s" variable to "<span class="remote-prefix"></span>".', 'migrations' ), $wpdb->prefix, $wpdb->prefix ); ?></p>

				<p><?php _e( 'This will allow your local install the use the migrated tables. Once you do this, you shouldn\'t have to do it again.', 'migrations' ); ?></p>
			</div>

			<div class="notification-message warning-notice prefix-notice push">
				<h4><?php _e( 'Warning: Different Table Prefixes', 'migrations' ); ?></h4>

				<p><?php printf( __( 'Whoa! We\'ve detected that the database table prefix differs between installations. Clicking the Migrate DB button below will create new database tables in the remote database with prefix "%s".', 'migrations' ), $wpdb->prefix ); ?></p>

				<p><?php printf( __( 'However, your remote install is configured to use table prefix "<span class="remote-prefix"></span>" and will ignore the migrated tables. So, <b>AFTER</b> migration is complete, you will need to edit your remote install\'s wp-config.php and change the "<span class="remote-prefix"></span>" variable to "%s".', 'migrations' ), $wpdb->prefix ); ?></p>

				<p><?php _e( 'This will allow your remote install the use the migrated tables. Once you do this, you shouldn\'t have to do it again.', 'migrations' ); ?></p>
			</div>
		...
		if( count( $this->settings['profiles'] ) > 0 ){ ?>
			<a href="<?php echo $this->plugin_base; ?>" class="return-to-profile-selection clearfix bottom">&larr; <?php _e( 'Back to select a saved profile', 'migrations' ); ?></a>
		<?php } ?>
