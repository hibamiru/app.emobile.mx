<div class="settings-tab content-tab">
	...
	<?php
		...
		$plugin_compatibility_checked = ( isset( $GLOBALS['mdb_compatibility'] ) ? ' checked="checked"' : '' );
	?>
	...
	<form method="post" id="settings-form" action="#settings" autocomplete="off">
		...
		<div class="option-section allow-remote-requests-wrap">
			<ul class="option-group">
				<li>
					...
					<?php _e( 'Accept <b>pull</b> requests allow this database to be exported and downloaded', 'migrations' ); ?>
					</label>
				</li>
				<li>
					<label for="allow_push">
					<input id="allow_push" type="checkbox" name="allow_push"<?php echo $push_checked; ?> />
					<?php _e( 'Accept <b>push</b> requests allow this database to be overwritten', 'migrations' ); ?>
					</label>
				</li>
				<li>
					...
					<?php _e( 'Enable SSL verification', 'migrations' ); ?>
					</label>
		...
		<div class="option-section connecton-info-wrap">
			<label for="connection_info" class="connection-info-label"><?php _e( 'Connection Info', 'migrations' ); ?></label>
			<textarea id="connection_info" class="connection-info" readonly><?php echo $connection_info; ?></textarea>
			<div class="reset-button-wrap clearfix"><a class="button reset-api-key js-action-link"><?php _e( 'Reset API Key', 'migrations' ); ?></a></div>
		</div>
		...
		<div class="option-section plugin-compatibility-section">
			...
			<?php _e( 'Improve performance and reliability by not loading the following plugins for migration requests', 'migrations' ); ?>
			</label>
			...
			<div class="plugin-compatibility-message helper-message bottom">
				<?php _e( 'Some plugins add a lot of overhead to each request, requiring extra memory and CPU. And some plugins even interfere with migrations and cause them to fail. We recommend only loading plugins that affect migration requests, for example a plugin that hooks into WP Sync DB.', 'migrations' ); ?></br>
			</div>
			...
			<div class="indent-wrap expandable-content plugin-compatibility-wrap select-wrap">
				...
				<br>
				<a class="multiselect-select-all js-action-link" href="#"><?php _e( 'Select All', 'migrations' ); ?></a>
				<span class="select-deselect-divider">/</span>
				<a class="multiselect-deselect-all js-action-link" href="#"><?php _e( 'Deselect All', 'migrations' ); ?></a>
				<span class="select-deselect-divider">/</span>
				<a class="multiselect-invert-selection js-action-link" href="#"><?php _e( 'Invert Selection', 'migrations' ); ?></a>

				<p>
					<span class="button plugin-compatibility-save"><?php _e( 'Save Changes', 'migrations' ); ?></span>
					<span class="plugin-compatibility-success-msg"><?php _e( 'Saved', 'migrations' ); ?></span>
				</p>
			</div>
		</div>
