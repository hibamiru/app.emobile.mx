(function($) {
  $.mdb = {
    ...
    add_action: function(action, callable, tag) {
      jQuery.mdb.add_hook('action', action, callable, tag);
    },
    add_filter: function(action, callable, tag) {
      jQuery.mdb.add_hook('filter', action, callable, tag);
    },
    do_action: function(action, args) {
      jQuery.mdb.do_hook('action', action, null, args);
    },
    apply_filters: function(action, value, args) {
      return jQuery.mdb.do_hook('filter', action, value, args);
    },
    remove_action: function(action, tag) {
      jQuery.mdb.remove_hook('action', action, tag);
    },
    remove_filter: function(action, tag) {
      jQuery.mdb.remove_hook('filter', action, tag);
    },
    add_hook: function(hook_type, action, callable, tag) {
      if (undefined == jQuery.mdb.hooks[hook_type][action]) {
        jQuery.mdb.hooks[hook_type][action] = [];
      }
      var hooks = jQuery.mdb.hooks[hook_type][action];
      if (undefined == tag) {
        tag = action + '_' + hooks.length;
      }
      jQuery.mdb.hooks[hook_type][action].push({
        tag: tag,
        callable: callable
      });
    },
