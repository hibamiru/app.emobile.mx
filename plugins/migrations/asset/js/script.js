  ...
  $(document).ready(function() {
    $('#plugin-compatibility').change(function(e) {
      ...
      $.ajax({
        ...
        cache: false,
        data: {
          action: 'mdb_plugin_compatibility',
          install: install,
        },
    ...
    $('.slider').slider({
      range: 'min',
      value: mdb_max_request / 1024,
      min: 512,
      max: mdb_bottleneck / 1024,
      step: 512,
      slide: function(event, ui) {
        $('.amount').html(mdb_add_commas(ui.value) + ' kB');
      },
      change: function(event, ui) {
        ...
        $.ajax({
          ...
          cache: false,
          data: {
            action: 'mdb_update_max_request_size',
            max_request_size: parseInt(ui.value),
            nonce: mdb_nonces.update_max_request_size,
          },
          error: function(jqXHR, textStatus, errorThrown) {
            $('.slider').slider('enable');
            $('.slider-spinner').remove();
            alert(mdb_i10n.max_request_size_problem);
          },
          success: function(data) {
            $('.slider').slider('enable');
            $('.slider-spinner').remove();
    ...
    $('.amount').html(mdb_add_commas($('.slider').slider('value')) +
      ' kB');
    ...
    function establish_remote_connection_from_saved_profile() {
      ...
      var connection_info = $.trim($('.pull-push-connection-info').val()).split(
        "\n");
      if (typeof mdb_default_profile == 'undefined' ||
        mdb_default_profile == true || action == 'savefile' || doing_ajax
      ) {
        return;
      }
      ...
      $('.connection-status').html(mdb_i10n.establishing_remote_connection);
      ...
      $.ajax({
        ...
        data: {
          ...
          intent: intent,
          nonce: mdb_nonces.verify_connection_to_remote_site,
          convert_post_type_selection: mdb_convert_post_type_selection,
          profile: mdb_profile
        },
        error: function(jqXHR, textStatus, errorThrown) {
          $('.connection-status').html(mdb_i10n.connection_local_server_problem +
            ' (#102)');
        ...
        success: function(data) {
          ...
          if (typeof data.mdb_error != 'undefined' && data.mdb_error ==
            1) {
          ...
          var loaded_tables = '';
          if (mdb_default_profile == false && typeof mdb_loaded_tables !=
            'undefined') {
            loaded_tables = mdb_loaded_tables;
          }
          ...
          if (mdb_default_profile == false && typeof mdb_loaded_post_types !=
            'undefined') {
            if (typeof data.select_post_types != 'undefined') {
              ...
              loaded_post_types = mdb_loaded_post_types;
            }
          }
          ...
          $.each(connection_data.post_types, function(index, value) {
            ...
            if (selected != -1 || (mdb_convert_exclude_revisions ==
              true && value != 'revision')) {
              selected = ' selected="selected" ';
            } else {
              selected = ' ';
          ...
          if (mdb_default_profile == false && typeof mdb_loaded_tables_backup !=
            'undefined') {
            loaded_tables_backup = mdb_loaded_tables_backup;
          }
          ...
          if ($('#pull').is(':checked')) {
            ...
            $('.uploads-dir').html(mdb_this_uploads_dir);
          } else {
            $('#select-backup').remove();
            $('.backup-tables-wrap').prepend(push_select_backup);
          }

          $.mdb.do_action('verify_connection_to_remote_site',
            connection_data);
    ...
    $('.clear-log').click(function() {
      ...
      $.ajax({
        ...
        cache: false,
        data: {
          action: 'mdb_clear_log',
          nonce: mdb_nonces.clear_log,
        },
        error: function(jqXHR, textStatus, errorThrown) {
          alert(mdb_i10n.clear_log_problem);
        },
        success: function(data) {}
      });
    });
    ...
    function refresh_debug_log() {
      $.ajax({
        ...
        data: {
          action: 'mdb_get_log',
          nonce: mdb_nonces.get_log,
        },
        error: function(jqXHR, textStatus, errorThrown) {
          alert(mdb_i10n.update_log_problem);
        },
        success: function(data) {
    ...
    $('#save-migration-profile').change(function() {
      if ($(this).is(':checked')) {
        ...
        $('.migrate-db .button-primary').val(mdb_i10n.migrate_db_save);
      } else {
        $('.save-settings-button').hide();
        $('.migrate-db .button-primary').val(mdb_i10n.migrate_db);
      }
    });
    ...
    $('.migrate-db-button').click(function(event) {
      ...
      if ($('#migrate-selected').is(':checked') && $('#select-tables').val() ==
        null) {
        alert(mdb_i10n.please_select_one_table);
        return;
      }
      ...
      if ($('#save-migration-profile').is(':checked')) {
        ...
        if ($.trim($('.create-new-profile').val()) == '' && $(
          ...
          alert(mdb_i10n.enter_name_for_profile);
        ...
        $.ajax({
          ...
          cache: false,
          data: {
            action: 'mdb_save_profile',
            profile: profile,
            nonce: mdb_nonces.save_profile,
          },
          error: function(jqXHR, textStatus, errorThrown) {
            alert(mdb_i10n.save_profile_problem);
          },
          success: function(data) {
      ...
      completed_msg = mdb_i10n.exporting_complete;

      if (migration_intent == 'savefile') {
        static_migration_label = mdb_i10n.exporting_please_wait;
      } else {
        static_migration_label = get_default_profile_name(remote_site,
          migration_intent, true) + ', ' + mdb_i10n.please_wait;
        completed_msg = get_default_profile_name(remote_site,
          migration_intent, true) + ' ' + mdb_i10n.complete;
      }
      ...
      if (stage == 'backup') {
        ...
        if (migration_intent == 'push') {
          ...
          table_rows = mdb_this_table_rows;
          if (backup_option == 'backup_only_with_prefix') {
            tables_to_migrate = mdb_this_prefixed_tables;
          } else if (backup_option == 'backup_selected') {
            selected_tables = $('#select-tables').val();
            tables_to_migrate = get_intersect(selected_tables,
              mdb_this_tables);
      ...
      $.ajax({
        ...
        cache: false,
        data: {
          action: 'mdb_initiate_migration',
          intent: migration_intent,
          url: remote_site,
          key: secret_key,
          form_data: form_data,
          stage: stage,
          nonce: mdb_nonces.initiate_migration,
        },
        error: function(jqXHR, textStatus, errorThrown) {
          $('.progress-title').html('Migration failed');
          $('.progress-text').html(mdb_i10n.connection_local_server_problem +
            ' (#112)');
        ...
        success: function(data) {
          if (typeof data.mdb_error != 'undefined' && data.mdb_error ==
            ...
            migration_error = true;
            migration_complete_events();
            $('.progress-title').html(mdb_i10n.migration_failed);
            $('.progress-text').addClass('migration-error');
          ...
          migrate_table_recursive = function(current_row, primary_keys) {
            if (i >= tables_to_migrate.length) {
              if (stage == 'backup') {
                ...
                hooks = $.mdb.apply_filters(
                  'mdb_before_migration_complete_hooks', hooks);
                hooks.push('migration_complete');
                hooks = $.mdb.apply_filters(
                  'mdb_after_migration_complete_hooks', hooks);
                hooks.push('migration_complete_events');
                next_step_in_migration = {
                  fn: mdb_call_next_hook
                };
                execute_next_step();
                return;
              }
            ...
            if (stage == 'backup') {
              $('.progress-text').html(overall_percent + '% - ' +
                mdb_i10n.backing_up + ' "' + tables_to_migrate[i] +
                '"');
            } else {
              $('.progress-text').html(overall_percent + '% - ' +
                mdb_i10n.migrating + ' "' + tables_to_migrate[i] +
                '"');
            }
            ...
            var request_data = {
              ...
              gzip: gzip,
              nonce: mdb_nonces.migrate_table,
            };
            ...
            $.ajax({
              ...
              error: function(jqXHR, textStatus, errorThrown) {
                $('.progress-title').html('Migration failed');
                $('.progress-text').html(mdb_i10n.table_process_problem +
                  ' ' + tables_to_migrate[i]);
                $('.progress-text').append('<br /><br />' +
                  mdb_i10n.status + ': ' + jqXHR.status + ' ' +
                  jqXHR.statusText + '<br /><br />' + mdb_i10n.response +
              ...
              success: function(data) {
                ...
                try {
                  ...
                  $('.progress-title').html('Migration failed');
                  if ('' == data || null == data) {
                    $('.progress-text').html(mdb_i10n.table_process_problem_empty_response +
                      ' ' + tables_to_migrate[i]);
                  } else {
                ...
                if (typeof row_information.mdb_error != 'undefined' &&
                  row_information.mdb_error == 1) {
                  $('.progress-title').html('Migration failed');
                  $('.progress-text').addClass('migration-error');
    ...
    migration_complete_events = function() {
      if (false == migration_error) {
        if (non_fatal_errors == '') {
          ...
          $('.progress-text').addClass('migration-error');
          $('.progress-title').html(mdb_i10n.completed_with_some_errors);
        }
    ...
    migration_complete = function() {
      ...
      if (migration_intent == 'savefile') {
        ...
        if ($('#save_computer').is(':checked')) {
          var url = mdb_this_download_url + encodeURIComponent(
            dump_filename);
          ...
          migrate_complete_text = mdb_i10n.completed_dump_located_at +
        ...
        $.ajax({
          ...
          data: {
            ...
            nonce: mdb_nonces.finalize_migration,
          },
          error: function(jqXHR, textStatus, errorThrown) {
            $('.progress-title').html(mdb_i10n.migration_failed);
            $('.progress-text').html(mdb_i10n.finalize_tables_problem);
            $('.progress-text').addClass('migration-error');
      ...
      $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
          ...
          $('.save-settings-button').after(
            '<span class="ajax-success-msg">' + mdb_i10n.saved +
            '</span>');
          $('.ajax-success-msg').fadeOut(2000, function() {
            $(this).remove();
          });
    ...
    function move_connection_info_box() {
      ...
      if ($('#pull').is(':checked')) {
        ...
        if (connection_established) {
          ...
          if (false == mdb_write_permission) {
            ...
            $('.upload-directory-location').html(mdb_this_upload_dir_long);
          }
        } else {
        ...
        if (connection_established) {
          $('.connection-status').hide();
          $('.step-two').show();
          $('.table-prefix').html(mdb_this_prefix);
      ...
      $.mdb.do_action('move_connection_info_box');
    }
    ...
    $('.reset-api-key').click(function() {
      var answer = confirm(mdb_i10n.reset_api_key);
      ...
      $.ajax({
        ...
        cache: false,
        data: {
          action: 'mdb_reset_api_key',
          nonce: mdb_nonces.reset_api_key,
        },
        error: function(jqXHR, textStatus, errorThrown) {
          alert(mdb_i10n.reset_api_key_problem);
    ...
    $('body').delegate('.delete-profile', 'click', function() {
      ...
      var answer = confirm(mdb_i10n.remove_profile + ' "' + name + '"');
      ...
      $.ajax({
        ...
        cache: false,
        data: {
          action: 'mdb_delete_migration_profile',
          profile_id: $(this).attr('data-profile-id'),
          nonce: mdb_nonces.delete_migration_profile,
        },
        error: function(jqXHR, textStatus, errorThrown) {
          alert(mdb_i10n.remove_profile_problem);
        },
        success: function(data) {
          if (data == '-1') {
            alert(mdb_i10n.remove_profile_not_found);
          }
        }
      });
    ...
    $('body').delegate('.temp-disabled', 'click', function() {
      var answer = confirm(mdb_i10n.change_connection_info);
      ...
      if (!answer) {
        ...
        $('.connect-button').show();
        $('.step-two').hide();
        $('.connection-status').show().html(mdb_i10n.enter_connection_info);
    ...
    $('.settings-tab input[type=checkbox]').change(function() {
      ...
      $.ajax({
        ...
        cache: false,
        data: {
          action: 'mdb_save_setting',
          checked: checked,
          setting: setting,
          nonce: mdb_nonces.save_setting,
        },
        error: function(jqXHR, textStatus, errorThrown) {
          alert(mdb_i10n.save_settings_problem);
          $('.ajax-spinner').remove();
        },
        success: function(data) {
          $('.ajax-spinner').remove();
    ...
    function connection_box_changed(data) {
      ...
      if (connection_info == '') {
        error = true;
        error_message = mdb_i10n.connection_info_missing;
      }

      if (connection_info.length != 2 && !error) {
        error = true;
        error_message = mdb_i10n.connection_info_incorrect;
      }

      if (!error && !validate_url(connection_info[0])) {
        error = true;
        error_message = mdb_i10n.connection_info_url_invalid;
      }

      if (!error && connection_info[1].length != 32) {
        error = true;
        error_message = mdb_i10n.connection_info_key_invalid;
      }

      if (!error && connection_info[0] == mdb_connection_info[0]) {
        error = true;
        error_message = mdb_i10n.connection_info_local_url;
      }

      if (!error && connection_info[1] == mdb_connection_info[1]) {
        error = true;
        error_message = mdb_i10n.connection_info_local_key;
      }
      ...
      if (mdb_openssl_available == false) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          if ($('#pull').is(':checked')) {
            $('#new-url').val(remove_protocol(mdb_this_url));
            $('#new-path').val(mdb_this_path);
            if (mdb_is_multisite == true) {
              $('#new-domain').val(mdb_this_domain);
            }
    ...
    $('body').delegate('.pause-resume', 'click', function() {
      if (true == migration_paused) {
        ...
        $('.pause-resume').html(mdb_i10n.pause);
        // resume the timer
        elapsed_interval = setInterval(count, 1000);
        execute_next_step();
      } else {
        ...
        $('.progress-title').html(mdb_i10n.migration_paused);
        $('.pause-resume').html(mdb_i10n.resume);
        $('.progress-text').html(mdb_i10n.completing_current_request);
      }
    });
    ...
    $('body').delegate('.cancel', 'click', function() {
      ...
      migration_paused = false;
      $('.progress-text').html(mdb_i10n.completing_current_request);
      $('.progress-title').html(mdb_i10n.cancelling_migration);
      $('.migration-controls').fadeOut();
      $('.migration-progress-ajax-spinner').show();
    ...
    execute_next_step = function() {
      if (true == migration_paused) {
        ...
        // pause the timer
        clearInterval(elapsed_interval);
        $('.progress-text').html(mdb_i10n.paused);
        return;
      } else if (true == migration_cancelled) {
        migration_intent = $('input[name=action]:checked').val();

        if ('savefile' == migration_intent) {
          progress_msg = mdb_i10n.removing_local_sql;
        } else if ('pull' == migration_intent) {
          if ('backup' == stage) {
            progress_msg = mdb_i10n.removing_local_backup;
          } else {
            progress_msg = mdb_i10n.removing_local_temp_tables;
          }
        } else if ('push' == migration_intent) {
          if ('backup' == stage) {
            progress_msg = mdb_i10n.removing_remote_sql;
          } else {
            progress_msg = mdb_i10n.removing_remote_temp_tables;
          }
        }
        $('.progress-text').html(progress_msg);

        var request_data = {
          action: 'mdb_cancel_migration',
          intent: migration_intent,
          url: remote_site,
          key: secret_key,
          stage: stage,
        ...
        $.ajax({
          ...
          data: request_data,
          error: function(jqXHR, textStatus, errorThrown) {
            $('.progress-title').html(mdb_i10n.migration_cancellation_failed);
            $('.progress-text').html(mdb_i10n.manually_remove_temp_files);
            $('.progress-text').append('<br /><br />Status: ' + jqXHR.status +
              ' ' + jqXHR.statusText + '<br /><br />Response:<br />' +
              jqXHR.responseText);
            $('.progress-text').addClass('migration-error');
          ...
          success: function(data) {
            ...
            completed_msg = mdb_i10n.migration_cancelled;
            $('.progress-text').hide();
            migration_complete_events();
          }
        });
