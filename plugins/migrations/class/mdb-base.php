...
class MDB_Base {
	...
	function __construct( $plugin_file_path ) {
		$this->settings = get_option( 'mdb_settings' );

		$this->addons = array(
			'migrations-media-files/migrations-media-files.php' => array(
				'name'				=> 'Media Files',
				'required_version'	=> '1.1.4b1',
			),
			'migrations-cli/migrations-cli.php' => array(
				'name'				=> 'CLI',
				'required_version'	=> '1.0b1',
			)
		);

		$this->invalid_content_verification_error = __( 'Invalid content verification signature, please verify the connection information on the remote site and try again.', 'migrations' );
		...
		$this->temp_prefix = apply_filters( 'mdb_temporary_prefix', $this->temp_prefix );
	}
	...
	function remote_post( $url, $data, $scope, $args = array(), $expecting_serial = false ) {
		...
		$default_remote_post_timeout = apply_filters( 'mdb_default_remote_post_timeout', 60 * 20 );
		...
		if ( is_wp_error( $response ) ) {
			if( strpos( $url, 'https://' ) === 0 && $scope == 'ajax_verify_connection_to_remote_site' ) {
				...
				$this->error = sprintf( __( 'The connection to the remote server has timed out, no changes have been committed. (#134 - scope: %s)', 'migrations' ), $scope );
			}
				...
				$this->error = sprintf( __( 'We could not find: %s. Are you sure this is the correct URL?', 'migrations' ), $_POST['url'] );
				$url_bits = parse_url( $_POST['url'] );
				if( strstr( $_POST['url'], 'dev.' ) || strstr( $_POST['url'], '.dev' ) || ! strstr( $url_bits['host'], '.' ) ) {
					$this->error .= '<br />';
					if( $_POST['intent'] == 'pull' ) {
						$this->error .= __( 'It appears that you might be trying to pull from a local environment. This will not work if <u>this</u> website happens to be located on a remote server, it would be impossible for this server to contact your local environment.', 'migrations' );
					}
					else {
						$this->error .= __( 'It appears that you might be trying to push to a local environment. This will not work if <u>this</u> website happens to be located on a remote server, it would be impossible for this server to contact your local environment.', 'migrations' );
					}
				}
			}
			else {
				$this->error = sprintf( __( 'The connection failed, an unexpected error occurred, please contact support. (#121 - scope: %s)', 'migrations' ), $scope );
			}
		...
		elseif ( (int) $response['response']['code'] < 200 || (int) $response['response']['code'] > 399 ) {
			if( strpos( $url, 'https://' ) === 0 && $scope == 'ajax_verify_connection_to_remote_site' ) {
				...
				$this->error = __( 'The remote site is protected with Basic Authentication. Please enter the username and password above to continue. (401 Unauthorized)', 'migrations' );
				$this->log_error( $this->error, $response );
				return false;
			}
			else {
				$this->error = sprintf( __( 'Unable to connect to the remote server, please check the connection details - %1$s %2$s (#129 - scope: %3$s)', 'migrations' ), $response['response']['code'], $response['response']['message'], $scope );
				$this->log_error( $this->error, $response );
				return false;
			}
		}
		elseif ( $expecting_serial && is_serialized( $response['body'] ) == false ) {
			...
			$this->error = __( 'There was a problem with the AJAX request, we were expecting a serialized response, instead we received:<br />', 'migrations' ) . htmlentities( $response['body'] );
			$this->log_error( $this->error, $response );
			return false;
		}
		elseif ( $response['body'] === '0' ) {
			...
			$this->error = sprintf( __( 'WP Sync DB does not seem to be installed or active on the remote site. (#131 - scope: %s)', 'migrations' ), $scope );
			$this->log_error( $this->error, $response );
			return false;
		}
	...
	function log_error( $mdb_error, $additional_error_var = false ){
		$error_header = "********************************************\n******  Log date: " . date( 'Y/m/d H:i:s' ) . " ******\n********************************************\n\n";
		$error = $error_header . "MDB Error: " . $mdb_error . "\n\n";
		...
		update_option( 'mdb_error_log', $log );
	}
	...
	function get_tables( $scope = 'regular' ) {
		...
		return apply_filters( 'mdb_tables', $clean_tables, $scope );
	}
	...
	function is_addon_outdated( $addon_basename ) {
		...
		if ( ! isset( $GLOBALS['mdb_meta'][$addon_slug]['version'] ) ) return true;
		$installed_version = $GLOBALS['mdb_meta'][$addon_slug]['version'];
	...
	function end_ajax( $return = false ) {
		if( defined( 'DOING_MDB_TESTS' ) || $this->doing_cli_migration ) {
	...
	function check_ajax_referer( $action ) {
		if ( defined( 'DOING_MDB_TESTS' ) || $this->doing_cli_migration ) return;
		$result = check_ajax_referer( $action, 'nonce', false );
		if ( false === $result ) {
			$return = array( 'mdb_error' => 1, 'body' => sprintf( __( 'Invalid nonce for: %s', 'migrations' ), $action ) );
			$this->end_ajax( json_encode( $return ) );
		}

		$cap = ( is_multisite() ) ? 'manage_network_options' : 'export';
		$cap = apply_filters( 'mdb_ajax_cap', $cap );
		if ( !current_user_can( $cap ) ) {
			$return = array( 'mdb_error' => 1, 'body' => sprintf( __( 'Access denied for: %s', 'migrations' ), $action ) );
			$this->end_ajax( json_encode( $return ) );
		}
	}
