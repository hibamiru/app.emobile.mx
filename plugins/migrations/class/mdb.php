...
class MDB extends MDB_Base {
	...
	function __construct( $plugin_file_path ) {
		...
		$this->plugin_slug = 'migrations';
		$this->plugin_version = $GLOBALS['mdb_meta']['migrations']['version'];
		...
		if( empty( $this->settings['max_request'] ) ) {
			...
			update_option( 'mdb_settings', $this->settings );
		}
		...
		add_action( 'wp_ajax_mdb_verify_connection_to_remote_site', array( $this, 'ajax_verify_connection_to_remote_site' ) );
		add_action( 'wp_ajax_mdb_reset_api_key', array( $this, 'ajax_reset_api_key' ) );
		add_action( 'wp_ajax_mdb_delete_migration_profile', array( $this, 'ajax_delete_migration_profile' ) );
		add_action( 'wp_ajax_mdb_save_setting', array( $this, 'ajax_save_setting' ) );
		add_action( 'wp_ajax_mdb_save_profile', array( $this, 'ajax_save_profile' ) );
		add_action( 'wp_ajax_mdb_initiate_migration', array( $this, 'ajax_initiate_migration' ) );
		add_action( 'wp_ajax_mdb_migrate_table', array( $this, 'ajax_migrate_table' ) );
		add_action( 'wp_ajax_mdb_finalize_migration', array( $this, 'ajax_finalize_migration' ) );
		add_action( 'wp_ajax_mdb_clear_log', array( $this, 'ajax_clear_log' ) );
		add_action( 'wp_ajax_mdb_get_log', array( $this, 'ajax_get_log' ) );
		add_action( 'wp_ajax_mdb_fire_migration_complete', array( $this, 'fire_migration_complete' ) );
		add_action( 'wp_ajax_mdb_update_max_request_size', array( $this, 'ajax_update_max_request_size' ) );
		add_action( 'wp_ajax_mdb_plugin_compatibility', array( $this, 'ajax_plugin_compatibility' ) );
		add_action( 'wp_ajax_mdb_blacklist_plugins', array( $this, 'ajax_blacklist_plugins' ) );
		add_action( 'wp_ajax_mdb_cancel_migration', array( $this, 'ajax_cancel_migration' ) );

		// external AJAX handlers
		add_action( 'wp_ajax_nopriv_mdb_verify_connection_to_remote_site', array( $this, 'respond_to_verify_connection_to_remote_site' ) );
		add_action( 'wp_ajax_nopriv_mdb_remote_initiate_migration', array( $this, 'respond_to_remote_initiate_migration' ) );
		add_action( 'wp_ajax_nopriv_mdb_process_chunk', array( $this, 'ajax_process_chunk' ) );
		add_action( 'wp_ajax_nopriv_mdb_process_pull_request', array( $this, 'respond_to_process_pull_request' ) );
		add_action( 'wp_ajax_nopriv_mdb_fire_migration_complete', array( $this, 'fire_migration_complete' ) );
		add_action( 'wp_ajax_nopriv_mdb_backup_remote_table', array( $this, 'respond_to_backup_remote_table' ) );
		add_action( 'wp_ajax_nopriv_mdb_remote_finalize_migration', array( $this, 'respond_to_remote_finalize_migration' ) );
		add_action( 'wp_ajax_nopriv_mdb_process_push_migration_cancellation', array( $this, 'respond_to_process_push_migration_cancellation' ) );
		...
		$this->rows_per_segment = apply_filters( 'mdb_rows_per_segment', $this->rows_per_segment );

		if ( is_multisite() ) {
			add_action( 'network_admin_menu', array( $this, 'network_admin_menu' ) );
			$this->plugin_base = 'settings.php?page=migrations';
		}
		else {
			add_action( 'admin_menu', array( $this, 'admin_menu' ) );
			$this->plugin_base = 'tools.php?page=migrations';
		}
	}
	...
	function ajax_plugin_compatibility() {
		...
		$source = trailingslashit( $this->plugin_dir_path ) . 'compatibility/migrations-compatibility.php';
		$dest = trailingslashit( $mu_dir ) . 'migrations-compatibility.php';
		if ( '1' === trim( $_POST['install'] ) ) { // install MU plugin
			if ( !wp_mkdir_p( $mu_dir ) ) {
				_e( sprintf( 'The following directory could not be created: %s', $mu_dir ), 'migrations' );
				exit;
			}
			if ( !copy( $source, $dest ) ) {
				_e( sprintf( 'Could not copy the compatibility plugin from %1$s to %2$s', $source, $destination ), 'migrations' );
				exit;
			}
		} else { // uninstall MU plugin
			if ( file_exists( $dest ) && !unlink( $dest ) ) {
				_e( sprintf( 'Could not remove the compatibility plugin from %s', $dest ), 'migrations' );
				exit;
			}
		}
		exit;
	...
	function check_again_clear_transients( $current_screen ) {
		...
		delete_site_transient( 'mdb_upgrade_data' );
	...
	function get_alter_table_name() {
		...
		global $wpdb;
		$this->alter_table_name = apply_filters( 'mdb_alter_table_name', $wpdb->prefix . 'mdb_alter_statements' );
		return $this->alter_table_name;
	}
	...
	function get_create_alter_table_query() {
		...
		$this->create_alter_table_query = apply_filters( 'mdb_create_alter_table_query', $this->create_alter_table_query );
		return $this->create_alter_table_query;
	}
	...
	function get_upload_info( $type = 'path' ) {
		...
		// );
		$upload_info = apply_filters( 'mdb_upload_info', array() );
		...
		$upload_dir_name = apply_filters( 'mdb_upload_dir_name', 'migrations' );

		if( ! file_exists( $upload_dir['basedir'] . DS . $upload_dir_name ) ) {
			$url = wp_nonce_url( $this->plugin_base, 'migrations-nonce' );
	...
	function parse_migration_form_data( $data ) {
		...
		$this->accepted_fields = apply_filters( 'mdb_accepted_profile_fields', $this->accepted_fields );
	...
	function plugin_action_links( $links ) {
		$link = sprintf( '<a href="%s">%s</a>', network_admin_url( $this->plugin_base ), __( 'Settings', 'migrations' ) );
	...
	function fire_migration_complete() {
		...
		do_action( 'mdb_migration_complete', 'pull', $_POST['url'] );
		$result = $this->end_ajax();
		return $result;
	}
	...
	function ajax_finalize_migration() {
		...
		if ( $_POST['intent'] == 'pull' ) {
			...
			do_action( 'mdb_migration_complete', 'push', $_POST['url'] );
			$data = $_POST;
			if ( isset( $data['nonce'] ) ) {
				unset( $data['nonce'] );
			}
			$data['action'] = 'mdb_remote_finalize_migration';
			$data['intent'] = 'pull';
	...
	function finalize_migration() {
		...
		$preserved_options = array( 'mdb_settings', 'mdb_error_log' );
		...
		$preserved_options = apply_filters( 'mdb_preserved_options', $preserved_options );
		...
		if( ! isset( $_POST['location'] ) ) {
			$data = array();
			$data['action'] = 'mdb_fire_migration_complete';
			$data['url'] = home_url();
		...
		do_action( 'mdb_migration_complete', $type, $location );
	}
	...
	function ajax_process_chunk() {
		...
		if ( !isset( $_FILES['chunk']['tmp_name'] ) || !move_uploaded_file( $_FILES['chunk']['tmp_name'], $tmp_file_path ) ) {
			$result = $this->end_ajax( __( 'Could not upload the SQL to the server. (#135)', 'migrations' ) );
			return $result;
		}

		if ( false === ( $chunk = file_get_contents( $tmp_file_path ) ) ) {
			$result = $this->end_ajax( __( 'Could not read the SQL file we uploaded to the server. (#136)', 'migrations' ) );
			return $result;
		}
		...
		if ( $this->settings['allow_push'] != true ) {
			$result = $this->end_ajax( __( 'The connection succeeded but the remote site is configured to reject push connections. You can change this in the "settings" tab on the remote site. (#133)', 'migrations' ) );
			return $result;
		}
	...
	function ajax_migrate_table() {
		...
		if ( $_POST['stage'] == 'backup' && $_POST['intent'] != 'savefile' ) {
			...
			if ( $_POST['intent'] == 'push' ) {
				...
				$data['action'] = 'mdb_backup_remote_table';
				$data['intent'] = 'pull';
				$ajax_url = trailingslashit( $_POST['url'] ) . 'wp-admin/admin-ajax.php';
		...
		if ( $_POST['intent'] == 'push' || $_POST['intent'] == 'savefile' ) {
			...
			$data['action'] = 'mdb_process_pull_request';
	...
	function respond_to_process_pull_request() {
		...
		if ( $this->settings['allow_pull'] != true ) {
			$result = $this->end_ajax( __( 'The connection succeeded but the remote site is configured to reject pull connections. You can change this in the "settings" tab on the remote site. (#132)', 'migrations' ) );
			return $result;
		}
	...
	function ajax_initiate_migration() {
		...
		if ( $_POST['intent'] == 'savefile' ) {
			...
			$data = array(
				'action'  => 'mdb_remote_initiate_migration',
				'intent' => $_POST['intent'],
			...
			if ( false === $response ) {
				$return = array( 'mdb_error' => 1, 'body' => $this->error );
			...
			if ( false === $return ) {
				$error_msg = __( 'Failed attempting to unserialize the response from the remote server. Please contact support.', 'migrations' );
				$return = array( 'mdb_error' => 1, 'body' => $error_msg );
			...
			if ( isset( $return['error'] ) && $return['error'] == 1 ) {
				$return = array( 'mdb_error' => 1, 'body' => $return['message'] );
				$result = $this->end_ajax( json_encode( $return ) );
				return $result;
			}
	...
	function respond_to_remote_initiate_migration() {
		...
		if ( $this->verify_signature( $filtered_post, $this->settings['key'] ) ) {
			if ( isset( $this->settings['allow_' . $_POST['intent']] ) && ( true === $this->settings['allow_' . $_POST['intent']] || 1 === $this->settings['allow_' . $_POST['intent']] ) ) {
				...
				$return['error'] = 1;
				if( $_POST['intent'] == 'pull' ) {
					$intent = __( 'pull', 'migrations' );
				}
				else {
					$intent = __( 'push', 'migrations' );
				}
				$return['message'] = sprintf( __( 'The connection succeeded but the remote site is configured to reject %s connections. You can change this in the "settings" tab on the remote site. (#110)', 'migrations'), $intent );
			}
		}
		else {
			$return['error'] = 1;
	...
	function ajax_verify_connection_to_remote_site() {
		...
		$data = array(
			'action'  => 'mdb_verify_connection_to_remote_site',
			'intent' => $_POST['intent']
		);

		$data['sig'] = $this->create_signature( $data, $_POST['key'] );
		$ajax_url = trailingslashit( $_POST['url'] ) . 'wp-admin/admin-ajax.php';
		$timeout = apply_filters( 'mdb_prepare_remote_connection_timeout', 10 );
		...
		if ( isset( $response['error'] ) && $response['error'] == 1 ) {
			$return = array( 'mdb_error' => 1, 'body' => $response['message'] );
	...
	function respond_to_verify_connection_to_remote_site() {
		...
		if ( !$this->verify_signature( $filtered_post, $this->settings['key'] ) ) {
			$return['error'] = 1;
			$return['message'] = $this->invalid_content_verification_error . ' (#120) <a href="#" class="try-again js-action-link">' . __( 'Try again?', 'migrations' ) . '</a>';
		...
		if ( !isset( $this->settings['allow_' . $_POST['intent']] ) || $this->settings['allow_' . $_POST['intent']] != true ) {
			...
			$return['message'] = sprintf( __( 'The connection succeeded but the remote site is configured to reject %s connections. You can change this in the "settings" tab on the remote site. (#122) <a href="#" class="try-again js-action-link">Try again?</a>', 'migrations' ), $intent );
			$result = $this->end_ajax( serialize( $return ) );
			return $result;
		}
		...
		$return = apply_filters( 'mdb_establish_remote_connection_data', $return );
		$result = $this->end_ajax( serialize( $return ) );
		return $result;
	}
	...
	function get_post_types() {
		...
		return apply_filters( 'mdb_post_types', array_unique( $return ) );
	}
	...
	function get_table_sizes( $scope = 'regular' ) {
		...
		return apply_filters( 'mdb_table_sizes', $return, $scope );
	}
	...
	function get_sensible_pull_limit() {
		return apply_filters( 'mdb_sensible_pull_limit', min( 26214400, $this->settings['max_request'] ) );
	}
	...
	function get_bottleneck( $type = 'regular' ) {
		...
		$post_max_upper_size = apply_filters( 'mdb_post_max_upper_size', 26214400 );
		...
		return apply_filters( 'mdb_bottleneck', $calculated_bottleneck );
	}
	...
	function options_page() {
		...
		<div class="wrap mdb">
			...
			<h2 class="nav-tab-wrapper"><a href="#" class="nav-tab nav-tab-active js-action-link migrate" data-div-name="migrate-tab"><?php _e( 'Migrate', 'migrations' ); ?></a><a href="#" class="nav-tab js-action-link settings" data-div-name="settings-tab"><?php _e( 'Settings', 'migrations' ); ?></a><a href="#" class="nav-tab js-action-link help" data-div-name="help-tab"><?php _e( 'Help', 'migrations' ); ?></a></h2>

			<?php do_action( 'mdb_notices' ); ?>

			<?php
			$hide_warning = apply_filters( 'mdb_hide_outdated_addons_warning', false );
			...
			foreach( $this->addons as $addon_basename => $addon ) {
				...
				if ( isset( $GLOBALS['mdb_meta'][$addon_slug]['version'] ) ) {
					$version = ' (' . $GLOBALS['mdb_meta'][$addon_slug]['version'] . ')';
				}
				else {
					$version = '';
				}
				...
				<div class="updated warning inline-message">
					...
					<?php printf( __( 'The version of the %1$s addon you have installed%2$s is out-of-date and will not work with this version WP Sync DB. <a href="%3$s">Update Now</a>', 'migrations' ), $addon['name'], $version, $update_url ); ?>
				</div>
			<?php
			}

			$hide_warning = apply_filters( 'mdb_hide_safe_mode_warning', false );
			if ( function_exists( 'ini_get' ) && ini_get( 'safe_mode' ) && !$hide_warning ) { ?>
				<div class="updated warning inline-message">
					<?php
					_e( "<strong>PHP Safe Mode Enabled</strong> &mdash; We do not officially support running this plugin in safe mode because <code>set_time_limit()</code> has no effect. Therefore we can't extend the run time of the script and ensure it doesn't time out before the migration completes. We haven't disabled the plugin however, so you're free to cross your fingers and hope for the best. However, if you have trouble, we can't help you until you turn off safe mode.", 'migrations' );
					if ( function_exists( 'ini_get' ) ) {
						printf( __( 'Your current PHP run time limit is set to %s seconds.', 'migrations' ), ini_get( 'max_execution_time' ) );
					} ?>
				</div>
				<?php
			}
			...
			<div class="updated warning ie-warning inline-message" style="display: none;">
				<?php _e( "<strong>Internet Explorer Not Supported</strong> &mdash; Less than 2% of our customers use IE, so we've decided not to spend time supporting it. We ask that you use Firefox or a Webkit-based browser like Chrome or Safari instead. If this is a problem for you, please let us know.", 'migrations' ); ?>
			</div>

			<?php
			$hide_warning = apply_filters( 'mdb_hide_set_time_limit_warning', false );
			if ( false == $this->set_time_limit_available() && !$hide_warning && !$safe_mode ) {
				?>
				<div class="updated warning inline-message">
					<?php
					_e( "<strong>PHP Function Disabled</strong> &mdash; The <code>set_time_limit()</code> function is currently disabled on your server. We use this function to ensure that the migration doesn't time out. We haven't disabled the plugin however, so you're free to cross your fingers and hope for the best. You may want to contact your web host to enable this function.", 'migrations' );
					if ( function_exists( 'ini_get' ) ) {
						printf( __( 'Your current PHP run time limit is set to %s seconds.', 'migrations' ), ini_get( 'max_execution_time' ) );
					} ?>
				</div>
			...
			<div id="mdb-main">
				...
				if ( ! empty( $this->settings['profiles'] ) && ! isset( $_GET['mdb-profile'] ) ) {
	...
	function apply_replaces( $subject, $is_serialized = false ) {
		 ...
		 */
		$subdomain_replace_enabled = apply_filters( 'mdb_subdomain_replace', true ); // allow developers to turn off this functionality
	...
	function export_table( $table ) {
		...
		if ( ! $table_structure ) {
			$this->error = __( 'Failed to retrieve table structure, please ensure your database is online. (#125)', 'migrations' );
			return false;
		}
		...
		if ( $current_row == -1 ) {
			...
			if ( $this->form_data['action'] == 'savefile' || $_POST['stage'] == 'backup' ) {
				$this->stow( "\n\n" );
				$this->stow( "#\n" );
				$this->stow( "# " . sprintf( __( 'Delete any existing table %s', 'migrations' ), $this->backquote( $table ) ) . "\n" );
				$this->stow( "#\n" );
				$this->stow( "\n" );
			...
			if ( $this->form_data['action'] == 'savefile' || $_POST['stage'] == 'backup' ) {
				$this->stow( "\n\n" );
				$this->stow( "#\n" );
				$this->stow( "# " . sprintf( __( 'Table structure of table %s', 'migrations' ), $this->backquote( $table ) ) . "\n" );
				$this->stow( "#\n" );
				$this->stow( "\n" );
			}
			...
			if ( false === $create_table ) {
				$this->error = __( 'Failed to generate the create table query, please ensure your database is online. (#126)', 'migrations' );
				return false;
			}
			...
			$create_table[0][1] = apply_filters( 'mdb_create_table_query', $create_table[0][1], $table );
			...
			if ( $this->form_data['action'] == 'savefile' || $_POST['stage'] == 'backup' ) {
				$this->stow( "\n\n" );
				$this->stow( "#\n" );
				$this->stow( '# ' . sprintf( __( 'Data contents of table %s', 'migrations' ), $this->backquote( $table ) ) . "\n" );
				$this->stow( "#\n" );
			}
		}
		...
		do {
			...
			if ( 'backup' != $_POST['stage'] && true === apply_filters( 'mdb_exclude_transients', true ) && isset( $this->form_data['exclude_transients'] ) && '1' === $this->form_data['exclude_transients'] && ( $this->table_is( 'options', $table ) || ( isset( $wpdb->sitemeta ) && $wpdb->sitemeta == $table ) ) ) {
			...
			$join = apply_filters( 'mdb_rows_join', $join, $table );
			$where = apply_filters( 'mdb_rows_where', $where, $table );
			$order_by = apply_filters( 'mdb_rows_order_by', $order_by, $table );
			$limit = apply_filters( 'mdb_rows_limit', $limit, $table );

			$sql = "SELECT " . $this->backquote( $table ) . ".* FROM " . $this->backquote( $table ) . " $join $where $order_by $limit";
			$sql = apply_filters( 'mdb_rows_sql', $sql, $table );
						...
						if ( isset( $ints[strtolower( $key )] ) && $ints[strtolower( $key )] ) {
							...
							if ( null === $value ) {
								...
								if( is_multisite() && 'domain' == $key && $_POST['stage'] != 'backup' && ( $wpdb->site == $table || $wpdb->blogs == $table ) ) {
									...
									$domain_replaces = apply_filters( 'mdb_domain_replaces', $domain_replaces );
		...
		if ( $this->form_data['action'] == 'savefile' || $_POST['stage'] == 'backup' ) {
			$this->stow( "\n" );
			$this->stow( "#\n" );
			$this->stow( "# " . sprintf( __( 'End of data contents of table %s', 'migrations' ), $this->backquote( $table ) ) . "\n" );
			$this->stow( "# --------------------------------------------------------\n" );
			$this->stow( "\n" );
	...
	function db_backup_header() {
		$charset = ( defined( 'DB_CHARSET' ) ? DB_CHARSET : 'utf8' );
		$this->stow( "# " . __( 'WordPress MySQL database migration', 'migrations' ) . "\n", false );
		$this->stow( "#\n", false );
		$this->stow( "# " . sprintf( __( 'Generated: %s', 'migrations' ), date( "l j. F Y H:i T" ) ) . "\n", false );
		$this->stow( "# " . sprintf( __( 'Hostname: %s', 'migrations' ), DB_HOST ) . "\n", false );
		$this->stow( "# " . sprintf( __( 'Database: %s', 'migrations' ), $this->backquote( DB_NAME ) ) . "\n", false );
		$this->stow( "# --------------------------------------------------------\n\n", false );
		...
		if ( $this->form_data['action'] == 'savefile' || $_POST['stage'] == 'backup' ) {
			if ( $this->gzip() && isset( $this->form_data['gzip_file'] ) ) {
				if ( ! @gzwrite( $this->fp, $query_line ) ) {
					$this->error = __( 'Failed to write the gzipped SQL data to the file. (#127)', 'migrations' );
					return false;
				}
			}
			else {
				if ( false === @fwrite( $this->fp, $query_line ) ) {
					$this->error = __( 'Failed to write the SQL data to the file. (#128)', 'migrations' );
					return false;
				}
			}
		}
	...
	function network_admin_menu() {
		$hook_suffix = add_submenu_page( 'settings.php', 'Migrate DB', 'Migrate DB', 'manage_network_options', 'migrations', array( $this, 'options_page' ) );
		$this->after_admin_menu( $hook_suffix );
	}

	function admin_menu() {
		$hook_suffix = add_management_page( 'Migrate DB', 'Migrate DB', 'export', 'migrations', array( $this, 'options_page' ) );
		$this->after_admin_menu( $hook_suffix );
	}
	...
	function load_assets() {
		...
		wp_enqueue_style( 'migrations-styles', $src, array(), $version );

		$src = $plugins_url . 'asset/js/common.js';
		wp_enqueue_script( 'migrations-common', $src, NULL, $version, true );

		$src = $plugins_url . 'asset/js/hook.js';
		wp_enqueue_script( 'migrations-hook', $src, NULL, $version, true );

		do_action( 'mdb_load_assets' );

		$src = $plugins_url . 'asset/js/script.js';
		wp_enqueue_script( 'migrations-script', $src, array( 'jquery' ), $version, true );

		wp_localize_script( 'migrations-script', 'mdb_i10n', array(
			'max_request_size_problem'				=> __( "A problem occurred when trying to change the maximum request size, please try again.", 'migrations' ),
			'establishing_remote_connection'		=> __( "Establishing connection to remote server, please wait", 'migrations' ),
			'connection_local_server_problem'		=> __( "A problem occurred when attempting to connect to the local server, please check the details and try again.", 'migrations' ),
			'clear_log_problem'						=> __( "An error occurred when trying to clear the debug log. Please contact support. (#132)", 'migrations' ),
			'update_log_problem'					=> __( "An error occurred when trying to update the debug log. Please contact support. (#133)", 'migrations' ),
			'migrate_db_save'						=> __( "Migrate DB & Save", 'migrations' ),
			'migrate_db'							=> __( "Migrate DB", 'migrations' ),
			'please_select_one_table'				=> __( "Please select at least one table to migrate.", 'migrations' ),
			'enter_name_for_profile'				=> __( "Please enter a name for your migration profile.", 'migrations' ),
			'save_profile_problem'					=> __( "An error occurred when attempting to save the migration profile. Please see the Help tab for details on how to request support. (#118)", 'migrations' ),
			'exporting_complete'					=> __( "Exporting complete", 'migrations' ),
			'exporting_please_wait'					=> __( "Exporting, please wait...", 'migrations' ),
			'please_wait'							=> __( "please wait...", 'migrations' ),
			'complete'								=> __( "complete", 'migrations' ),
			'migration_failed'						=> __( "Migration failed", 'migrations' ),
			'backing_up'							=> __( "Backing up", 'migrations' ),
			'migrating'								=> __( "Migrating", 'migrations' ),
			'status'								=> __( "Status", 'migrations' ),
			'response'								=> __( "Response", 'migrations' ),
			'table_process_problem'					=> __( "A problem occurred when attempting to process the following table (#113)", 'migrations' ),
			'table_process_problem_empty_response'	=> __( "A problem occurred when processing the following table. We were expecting a response in JSON format but instead received an empty response.", 'migrations' ),
			'completed_with_some_errors'			=> __( "Migration completed with some errors", 'migrations' ),
			'completed_dump_located_at'				=> __( "Migration complete, your backup is located at:", 'migrations' ),
			'finalize_tables_problem'				=> __( "A problem occurred when finalizing the backup. (#132)", 'migrations' ),
			'saved'									=> __( "Saved", 'migrations' ),
			'reset_api_key'							=> __( "Any sites setup to use the current API key will no longer be able to connect. You will need to update those sites with the newly generated API key. Do you wish to continue?", 'migrations' ),
			'reset_api_key_problem'					=> __( "An error occurred when trying to generate the API key. Please see the Help tab for details on how to request support. (#105)", 'migrations' ),
			'remove_profile'						=> __( "You are removing the following migration profile. This cannot be undone. Do you wish to continue?", 'migrations' ),
			'remove_profile_problem'				=> __( "An error occurred when trying to delete the profile. Please see the Help tab for details on how to request support. (#106)", 'migrations' ),
			'remove_profile_not_found'				=> __( "The selected migration profile could not be deleted because it was not found.\nPlease refresh this page to see an accurate list of the currently available migration profiles.", 'migrations' ),
			'change_connection_info'				=> __( "If you change the connection details, you will lose any replaces and table selections you have made below. Do you wish to continue?", 'migrations' ),
			'enter_connection_info'					=> __( "Please enter the connection information above to continue.", 'migrations' ),
			'save_settings_problem'					=> __( "An error occurred when trying to save the settings. Please try again. If the problem persists, please see the Help tab for details on how to request support. (#108)", 'migrations' ),
			'connection_info_missing'				=> __( "The connection information appears to be missing, please enter it to continue.", 'migrations' ),
			'connection_info_incorrect'				=> __( "The connection information appears to be incorrect, it should consist of two lines. The first being the remote server's URL and the second being the secret key.", 'migrations' ),
			'connection_info_url_invalid'			=> __( "The URL on the first line appears to be invalid, please check it and try again.", 'migrations' ),
			'connection_info_key_invalid'			=> __( "The secret key on the second line appears to be invalid. It should be a 32 character string that consists of letters, numbers and special characters only.", 'migrations' ),
			'connection_info_local_url'				=> __( "It appears you've entered the URL for this website, you need to provide the URL of the remote website instead.", 'migrations' ),
			'connection_info_local_key'				=> __( "It appears you've entered the secret key for this website, you need to provide the secret key for the remote website instead.", 'migrations' ),
			'time_elapsed'							=> __( "Time Elapsed:", 'migrations' ),
			'pause'									=> __( "Pause", 'migrations' ),
			'migration_paused'						=> __( "Migration Paused", 'migrations' ),
			'resume'								=> __( "Resume", 'migrations' ),
			'completing_current_request'			=> __( "Completing current request", 'migrations' ),
			'cancelling_migration'					=> __( "Cancelling migration", 'migrations' ),
			'paused'								=> __( "Paused", 'migrations' ),
			'removing_local_sql'					=> __( "Removing the local MySQL export file", 'migrations' ),
			'removing_local_backup'					=> __( "Removing the local backup MySQL export file", 'migrations' ),
			'removing_local_temp_tables'			=> __( "Removing the local temporary tables", 'migrations' ),
			'removing_remote_sql'					=> __( "Removing the remote backup MySQL export file", 'migrations' ),
			'removing_remote_temp_tables'			=> __( "Removing the remote temporary tables", 'migrations' ),
			'migration_cancellation_failed'			=> __( "Migration cancellation failed", 'migrations' ),
			'manually_remove_temp_files'			=> __( "A problem occurred while cancelling the migration, you may have to manually delete some temporary files / tables.", 'migrations' ),
			'migration_cancelled'					=> __( "Migration cancelled", 'migrations' ),
		) );
	...
	function download_file() {
		...
		if ( file_exists( $diskfile ) ) {
			...
			wp_die( __( 'Could not find the file to download:', 'migrations' ) . '<br />' . $diskfile );
		}
	}
	...
	function admin_head_connection_info() {
		...
		$nonces = apply_filters( 'mdb_nonces', $nonces );

		?>
		<script type='text/javascript'>
			var mdb_connection_info = <?php echo json_encode( array( site_url( '', 'https' ), $this->settings['key'] ) ); ?>;
			var mdb_this_url = '<?php echo addslashes( home_url() ) ?>';
			var mdb_this_path = '<?php echo addslashes( $this->absolute_root_file_path ); ?>';
			var mdb_this_domain = '<?php echo $this->get_domain_current_site(); ?>';
			var mdb_this_tables = <?php echo json_encode( $this->get_tables() ); ?>;
			var mdb_this_prefixed_tables = <?php echo json_encode( $this->get_tables( 'prefix' ) ); ?>;
			var mdb_this_table_sizes = <?php echo json_encode( $this->get_table_sizes() ); ?>;
			var mdb_this_table_rows = <?php echo json_encode( $this->get_table_row_count() ); ?>;
			var mdb_this_upload_url = '<?php echo addslashes( trailingslashit( $this->get_upload_info( 'url' ) ) ); ?>';
			var mdb_this_upload_dir_long = '<?php echo addslashes( trailingslashit( $this->get_upload_info( 'path' ) ) ); ?>';
			var mdb_this_website_name = '<?php echo sanitize_title_with_dashes( DB_NAME ); ?>';
			var mdb_this_download_url = '<?php echo network_admin_url( $this->plugin_base . '&download=' ); ?>';
			var mdb_this_prefix = '<?php echo $table_prefix; ?>';
			var mdb_is_multisite = <?php echo ( is_multisite() ? 'true' : 'false' ); ?>;
			var mdb_openssl_available = <?php echo ( $this->open_ssl_enabled() ? 'true' : 'false' ); ?>;
			var mdb_plugin_version = '<?php echo $this->plugin_version; ?>';
			var mdb_max_request = '<?php echo $this->settings['max_request'] ?>';
			var mdb_bottleneck = '<?php echo $this->get_bottleneck( 'max' ); ?>';
			var mdb_this_uploads_dir = '<?php echo addslashes( $this->get_short_uploads_dir() ); ?>';
			var mdb_write_permission = <?php echo ( is_writeable( $this->get_upload_info( 'path' ) ) ? 'true' : 'false' ); ?>;
			var mdb_nonces = <?php echo json_encode( $nonces ); ?>;
			var mdb_profile = '<?php echo ( isset( $_GET['mdb-profile'] ) ? $_GET['mdb-profile'] : '-1' ); ?>';
			<?php do_action( 'mdb_js_variables' ); ?>
		</script>
		<?php
	}
	...
	function ajax_cancel_migration() {
		...
		switch( $_POST['intent'] ) {
			...
			case 'push' :
				$data = $_POST;
				$data['action'] = 'mdb_process_push_migration_cancellation';
				$data['temp_prefix'] = $this->temp_prefix;
				$ajax_url = trailingslashit( $data['url'] ) . 'wp-admin/admin-ajax.php';
				$data['sig'] = $this->create_signature( $data, $data['key'] );
	...
	function delete_export_file( $filename, $is_backup ) {
		...
		if( empty( $dump_file ) || false == file_exists( $dump_file ) ) {
			_e( 'MySQL export file not found.', 'migrations' );
			exit;
		}

		if( false === @unlink( $dump_file ) ) {
			e( 'Could not delete the MySQL export file.', 'migrations' );
			exit;
		}
	}
