...
function user_registration_form( $email ) {
	global $display_form;
	if ($display_form) {
		echo '
		<style>
...
function user_registration_validation( $email )  {
    global $reg_errors;
...
add_shortcode('registro_usuarios', 'custom_user_registration_shortcode');

// The callback function that will register new user
function custom_user_registration_shortcode() {
    ob_start();
    custom_user_registration_function();
    return ob_get_clean();
}
